...
# live URLs by default; override with WEB and API. Probes the dual-stack
# Bedrock server named in LIVE_BEDROCK_HOST (a repository secret in CI) and
# Hypixel (IPv4 Java), so those must be up. Without LIVE_BEDROCK_HOST the
# Bedrock checks are skipped. The server's addresses come from the API's
# answers and are masked in CI logs.
#
#   LIVE_BEDROCK_HOST=<name> sh test/live.sh
...
if [ -n "$HOST" ]; then
    ...
    check "ping: name, IPv4 online"     [ -n "$V4" ]
    check "ping: name, IPv6 online"     [ -n "$V6" ]
    check "ping: v4 literal"            json "$API/ping?ip=$V4&port=19132&host=$HOST" "d['state'] == 'online'"
    check "ping: v6 literal bare"       json "$API/ping?ip=$V6&port=19132&host=$HOST" "d['state'] == 'online'"
    check "ping: v6 literal bracketed"  json "$API/ping?ip=%5B$V6%5D&port=19132&host=$HOST" "d['state'] == 'online'"
    check "ping: second call cached"    json "$API/ping?ip=$V4&port=19132&host=$HOST" "d.get('cached') and d['age_s'] >= 0"
else
    echo "skip  Bedrock checks: LIVE_BEDROCK_HOST is not set"
fi
check "ping: java hostname"         json "$API/ping?host=$JAVA_HOST&family=4&port=25565&edition=java" "d['state'] == 'online' and d['info']['players_max'] > 0"
check "ping: v4-only host, IPv6"    json "$API/ping?host=$JAVA_HOST&family=6&port=25565&edition=java" "d['state'] == 'no_dns'"
...
check "cors header for any origin"  sh -c "curl -s -D - -o /dev/null -H 'Origin: https://example.org' '$API/ping?ip=nope&port=1' | grep -qi 'access-control-allow-origin: https://example.org'"
